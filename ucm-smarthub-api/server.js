...
// Cursos válidos da UCM Extensão de Tete
const CURSOS_VALIDOS = [
  "Gestão de Empresas","Contabilidade e Auditoria","Economia",
  "Direito","Informática","Engenharia Civil",
  "Enfermagem","Análises Clínicas",
  "Ensino de Matemática","Ensino de Biologia","Ensino de Física",
  "Ensino de Química","Ensino de Português","Filosofia","Psicologia",
];

// Modelos a tentar por ordem — gemini-2.5-flash é o que estava a funcionar no projecto
const GEMINI_MODELS = [
  "gemini-2.5-flash",
  "gemini-2.0-flash",
  "gemini-1.5-flash",
...
// ==========================================
// MULTER — limite de 100 MB por ficheiro
// ==========================================
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    if (TIPOS_PERMITIDOS.includes(file.mimetype)) {
      ...
      cb(new Error("Tipo de ficheiro não permitido. Aceite apenas PDF e vídeo."));
    }
  },
});
...
app.post("/api/register", limitarRegisto, async (req, res) => {
  try {
    ...
    const papel = ["estudante", "professor"].includes(req.body.papel)
      ? req.body.papel
      : "estudante";
    const cursoValido = CURSOS_VALIDOS.includes(req.body.curso) ? req.body.curso : "Informática";
  ...
  try {
    ...
    if (material.tipo === "PDF") {
      ...
      const promptResumo = temTexto
        ? `És o UCM Brain, assistente académico do UCM SmartHub (Universidade Católica de Moçambique, Extensão de Tete).
...
Conteúdo:
${trimmedText}`
        : `És o UCM Brain, assistente académico do UCM SmartHub (UCM Extensão de Tete).
...
Leia o material duas vezes: primeiro para compreensão geral, depois sublinhando os conceitos-chave. Crie um mapa mental ligando os tópicos principais antes de resolver exercícios práticos.`;
      ...
      // Vídeo ou outro tipo
      const promptResumo = `És o UCM Brain, assistente académico do UCM SmartHub (UCM Extensão de Tete).
...
app.post("/api/materiais", autenticar, upload.single("arquivo"), async (req, res) => {
  ...
  try {
    ...
    if (!CURSOS_VALIDOS.includes(cadeira)) {
      limparFicheiroOrfao();
    ...
    const [resultado] = await db.query(
      "INSERT INTO materiais (titulo, cadeira, tipo, url_arquivo, autor_id, status) VALUES (?, ?, ?, ?, ?, ?)",
      [titulo, cadeira, tipo, url_arquivo, autor_id, "pendente"]
    );
...
app.get("/api/admin/pendentes", autenticar, apenasAdmin, async (req, res) => {
  try {
    const [materiais] = await db.query(
      `SELECT m.id, m.titulo, m.cadeira, m.tipo, m.data_upload, u.nome AS autor
...
app.get("/api/stats", autenticar, async (req, res) => {
  try {
    ...
    res.json({ total_materiais, total_utilizadores, total_mensagens, total_pdfs, total_videos, materiais_hoje });
  } catch (erro) {
...
app.post("/api/chat", autenticar, async (req, res) => {
  try {
    ...
    const prompt = `És o UCM Brain — assistente académico oficial do UCM SmartHub da Universidade Católica de Moçambique, Extensão de Tete.
...
- Quando relevante, usar exemplos do contexto moçambicano ou africano
