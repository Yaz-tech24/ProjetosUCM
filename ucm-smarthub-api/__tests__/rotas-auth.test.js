...
describe("Sessão via cookie httpOnly", () => {
  ...
  it("GET /api/me autentica pelo cookie de sessão, sem cabeçalho Authorization", async () => {
    ...
    mockSql([[/SELECT id, nome, email, papel, curso, numero_estudante, telefone, avatar_url, email_verificado, 2fa_ativado, digest_semanal FROM usuarios WHERE id/, [[{ id: 7, nome: "Carlos", email: "c@d.com", papel: "estudante", curso: "Geral", avatar_url: null }]]]]);
  ...
  it("GET /api/me continua a aceitar Authorization: Bearer (clientes de API/Swagger)", async () => {
    ...
    mockSql([[/SELECT id, nome, email, papel, curso, numero_estudante, telefone, avatar_url, email_verificado, 2fa_ativado, digest_semanal FROM usuarios WHERE id/, [[{ id: 9, nome: "Bea", email: "bea@teste.com", papel: "estudante", curso: "Geral", avatar_url: null }]]]]);
